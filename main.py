...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    ...
    for batch_idx, (image, target) in enumerate(data_loader):
        ...
        if last_batch or batch_idx % args.print_freq == 0:
            ...
            loss_logger.update(reduced_loss.item(), batch_size)
            top1_logger.update(acc1.item(), batch_size)
            top5_logger.update(acc5.item(), batch_size)

            if args.rank == 0:
                print('Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
                      'Loss: {loss.val:>4.4f} ({loss.avg:>4.3f})  '
                      'Acc@1: {acc1.val:>4.4f} ({acc1.avg:>4.4f}) '
                      'Acc@5: {acc5.val:>4.4f} ({acc5.avg:>4.4f}) '
                      'LR: {lr:.3e}  '
...
def main(args):
    ...
    from nets.nn import efficientnet_b0
    ...
    if args.model_ema:
        ...
        model_ema = utils.EMA(model_without_ddp, decay=1.0 - alpha)
